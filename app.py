...
def load_data():
    ...
    with DATA_LOCK:
        # 1. Primary: Load from Parquet (Optimized for Vercel)
        if os.path.exists(CACHE_COMP) and os.path.exists(CACHE_SPEC):
            try:
                ...
                IS_LOADED = True
                print("✅ Data loaded from Parquet. TSVs not required.", file=sys.stderr)
                return
            except Exception as e:
                print(f"⚠️ Parquet error: {e}", file=sys.stderr)

        # 2. Fallback: Local Generation (Only if Parquet is missing)
        try:
            print("📂 Processing TSVs to generate Parquet cache...", file=sys.stderr)
            ...
            IS_LOADED = True
            print("✅ Parquet cache created. You can now delete TSVs and push to GitHub.", file=sys.stderr)
...
def api_comp():
    ...
    # Fix: Ensure NumPy arrays from Parquet are converted to lists for JSON
    return jsonify([{
        "personId": r['wca_id'], 
        "personName": r['name'], 
